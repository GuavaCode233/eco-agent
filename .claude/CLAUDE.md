...
> **能耗模型（v20 [D7]）**：棄「活躍時間 × TDP」。Agent **純感測、只送原始量**，能耗由後端以使用率加權模型 `P_idle + 使用率 ×(P_active − P_idle)` 計算。誘因對齊「節能」（歸戶 idle 開機的浪費）而非「少用」。
...
> 依 v20：路徑 B 歸戶前提已定案。**本階段只做「個人專屬印表機 SNMP 輪詢歸戶」一軌**；共用機的 Print Server Log / Pull Printing API 標「未來實作」不在範圍；手動上傳用紙量屬 App 端、非 Agent 路徑。
...
> **同步提醒**：下表數值複製自 v20 §4.4.4，屬複製關係。v20 若改動任一值，本表須一併同步。
...
- **預留串接空間**：把「索取 binding_code → 顯示 QR（custom scheme URI，見 v20 §4.5）→ 等 App 掃碼 → 後端發雙 token → 存金鑰庫」整條流程寫成一個帶 `// TODO(backend)` 的 stub 函式與註解，日後後端就緒即可填。
