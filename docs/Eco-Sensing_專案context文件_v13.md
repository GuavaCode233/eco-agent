...
| 路徑 | 對象 | 方法 | 協定 |
|------|------|------|------|
| A | 電腦使用 | Windows `GetLastInputInfo()` / macOS `IOHIDGetModifierLockState()` 判活躍/閒置＋跨平台 CPU 使用率（`gopsutil`），每固定區間 `computerUsageRecordInterval` 輪詢；**Agent 只送原始量（active/idle 時數、平均使用率、CPU 型號），能耗由後端以使用率加權模型計算**（見「電腦能耗模型」） | MQTT |
